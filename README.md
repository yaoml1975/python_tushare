...
- **四大筛选模块**：从流动性、估值水平、财务指标到技术面的递进式筛选
- **数据整合**：整合Tushare Pro的行情、财务、基本面等多维度数据
- **自动化流程**：支持从数据获取到最终选股的全流程自动化
- **可配置策略**：通过config.yaml文件灵活调整筛选参数

## 💖 支持开源
**您的支持是本项目持续优化与开源的最大动力！**  
如果您愿意支持我们，可通过微信扫码捐赠。所有捐赠将用于：
- 🛠️ **代码优化**：改进算法性能与策略准确性
- 📊 **数据维护**：支付Tushare Pro接口费用及服务器成本
- 🚀 **功能扩展**：开发回测框架、GUI界面等新功能
- 📚 **开源生态**：完善文档及社区技术支持
